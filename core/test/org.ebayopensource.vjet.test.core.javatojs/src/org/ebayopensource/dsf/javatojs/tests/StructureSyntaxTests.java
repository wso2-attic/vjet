...
//					ctx.getConfig().getPackageMapping().add("org", "vjo");
	...
	public void testArray() {
		...
		assertEquals(0, TestHelper.getTranslationController(getInitializer()).getErrors().size());
	}
	...
	public void testEnum() {
		TestHelper.testType(org.ebayopensource.dsf.javatojs.tests.data.structure.Enum.class,getInitializer());
	}
	...
	public void testGenerics() {
		//TODO - The Date usage in Generics.java is not handled correctly now for JSR
		//		Needs to be fixed once decided on the usage of Date since translation is not available
		...
		JstType type = JstCache.getInstance().getType("org.ebayopensource.dsf.javatojs.tests.data.structure.Generics");
		...
		assertEquals("Date", type.getParamType("Ctx2").getBounds().get(0).getSimpleName());

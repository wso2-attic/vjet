...
public class JstTypeHelper {
	...
	public static boolean isTypeOf(IJstType aType, IJstType bType){
		if (aType instanceof IJstRefType &&
			"Function".equals(bType.getName())) {
			return true;
		}
		return 
			isSubTypeOf(aType, bType) 
			|| isImplementerOf(aType, bType) 
...
//	 TODO: make it complete
	...
	/**
	 * Resolve parameter type e.g. E in ArrayList<E> to real type, e.g. String in ArrayList<String>
	 * @param paramType - parameter type, e.g. <E>
	 * @param parentType - type with arguments, e.g. ArrayList<String>
	 * @return IJstType - resolved param type, e.g. String
	 * 
	 * paramType can be a method or property in the future and this function will get the method return
	 * type or property type. 
	 * 
	 * -- TODO: Future implementation should handle the following scenario: 
	 * 
	 * For example:
	 * 
	 * 1. Two generic class B<X, Y, Z> and A<E, F>
	 * 2. B<X, Y, Z> inherits A<Y, Z> or A<String, Integer>
	 * 3. A has a method foo() returns E type
	 * 4. B<boolean, String, Integer> is instanciated to call foo()
	 * 5. How to resolve E? E should be resolved to String type
	 */
	public static IJstType resolveTypeWithArgs(IJstNode mtdOrPty, IJstType parentType) {
		...
		if (paramType instanceof JstParamType) {
			if (parentType instanceof JstTypeWithArgs) {
				return getParamType((JstParamType) paramType, (JstTypeWithArgs)parentType);
			}
			else {
	...
	private static IJstType getParamType(JstParamType paramType, JstTypeWithArgs typeWithArgs) {
		...
		/*
		int index  = next.getType().getParamTypes().indexOf(paramType);
		if (index >= 0 && next.getArgTypes().size() > 0) {						
			return next.getArgTypes().get(index);
		}
		else if(typeWithArgs.getExtend() != null && typeWithArgs.getExtend() instanceof JstTypeWithArgs){
			final IJstType resolvedNextType = getParamType(paramType, typeWithArgs, (JstTypeWithArgs)typeWithArgs.getExtend());
			//modification by roy
			//if the ext resolved type is a JstParamType, the root JstTypeWithArgs should have the correct mapping
			//otherwise, returns anyway
			if(resolvedNextType instanceof JstParamType){
				index = typeWithArgs.getType().getParamTypes().indexOf((JstParamType)resolvedNextType);
				if (index >= 0 && typeWithArgs.getArgTypes().size() > 0) {						
					return typeWithArgs.getArgTypes().get(index);
				}
			}
			else{
		...
		//TODO should interfaces param types be considered as well?
	...
	/**
	 * Return the actual IJstType for input method argument. If the method argument type is defined as generic type param,
	 * then the parameter type e.g. 'E' will be replaced by the corresponding real type e.g. 'String' from the
	 * typeWithArgs type.
	 * @param arg - JstARg
	 * @param typeWithArgs - JstTypeWithArgs
	 * @return List<IJstType>
	 * 
	 * For example, type GenericTest<E> has method bar(int, E, boolean) as mtd parameter,
	 * 
	 * vjo.ctype('vjo.vjet.GenericTest<E>') //< public
	 * .props({
	 *})
	 *.protos({
	 * >>public E bar(int i, E a, boolean t)
	 * bar:function(i, a, t) {
	 * return null;
	 * }
	 * })
	 * .endType();
	 * 
	 * With GenericTest<String> as typeWithArgs parameter, the return array is [int, String, boolean]
	 */
	public static IJstType resolveJstArgType(JstArg arg, IJstType typeWithArgs) {
		...
		if (argType instanceof JstParamType && typeWithArgs instanceof JstTypeWithArgs) {
			argType = getParamType((JstParamType)argType, (JstTypeWithArgs)typeWithArgs);
	...
	/**
	 * Answer a list of all constructors defined via signature
	 * The list includes overloaded constructors but not the dispatcher constructor.
	 * @param IJstType 
	...
	/**
	 * Answer a list of all methods defined via signature
	 * The list includes overloaded methods but not the dispatch method.
	 * The list includes base type(s) methods if "recursive" is true.
	 * @param type IJstType
	 * @param recursive boolean if true, list includes base type(s) signature methods
	...
	/**
	 * Temp helper method to get the package for inner types since
	 * JstType returns incorrect package for inner type .
	 * @param type
	 * @return
	 */
...
//		}
	...
	public static boolean isConstructor(IJstNode node) {
		...
		if (node instanceof JstConstructor) {
			...
			if (method != null) {
				...
				if (mtdName != null && mtdName.startsWith(JstConstructor.CONSTRUCTS) && 
	...
	public static void populateMethod(JstMethod to, IJstMethod from) {
		...
		for (JstArg arg : args) {
			to.addArg(new JstArg(arg.getType(), arg.getName(), arg.isVariable(),
					arg.isOptional(), arg.isFinal()));
		}
	...
	public static void populateMethod(JstMethod to, IJstMethod from, boolean keepArgNames) {
		...
		for (JstArg arg : args) {
			...
			to.addArg(new JstArg(arg.getType(), argName, arg.isVariable(),
					arg.isOptional(), arg.isFinal()));
			idx ++;
		}
		if (to.getParamTypes()!=null) {

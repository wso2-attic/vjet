...
if(s){r=gx.exec(s);if(r){l=r[0].length;s=s.substr(l);character+=l;globals[r[1]]=9;}else{error("Bad extern identifier: '"+
...
error("Bad regular expression: "+s);}};}();function builtin(name){if(standard[name]||globals[name]){return true;}
if(xtype==='widget'){return konfab[name];}
if(xtype==='html'||option.browser){return browser[name];}
return false;}
...
function suffix(s,f){var x=symbol(s,150);x.led=function(left){if(option.plusplus){warning("This operator is considered harmful: "+this.id,this);}

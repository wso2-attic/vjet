...
import vjo.lang.*;
import vjo.lang.Long;
import vjo.lang.Short;
import vjo.lang.Integer;
import vjo.lang.StringBuilder;

import vjo.lang.Character;

import vjo.util.Calendar;
import vjo.util.GregorianCalendar;
import vjo.util.HashMap;
import vjo.util.Locale;
import vjo.util.ResourceBundle;
import vjo.util.TimeZone;
import vjo.sun.text.resources.LocaleData;
...
public class BuddhistCalendar extends GregorianCalendar {
	...
	/**
	 * Constructs a default BuddhistCalendar using the current time in the
	...
	/**
	 * Constructs a BuddhistCalendar based on the current time in the given time
	...
	/**
	 * Constructs a BuddhistCalendar based on the current time in the default
	 ...
	 * @return true if this object is equal to <code>obj</code>; false
	 *         otherwise
	 */
	...
	/**
	 * Add to field a signed amount without changing larger fields. A negative
	 * roll amount means to subtract from field without changing larger fields.
	 * 
	 * @param field
	 *            the time field.
	 * @param amount
	...
	public String getDisplayName(int field, int style, Locale locale) {
		...
		if (field < 0 || field >= fields.length || style < SHORT
				|| style > LONG) {
	...
	public Map<String, Integer> getDisplayNames(int field, int style,
		...
		// Handle Thai BuddhistCalendar specific era names
		if (field < 0 || field >= fields.length || style < ALL_STYLES
				|| style > LONG) {
			throw new IllegalArgumentException();
		}
		if (locale == null) {
		...
		// ALL_STYLES
		if (style == ALL_STYLES) {
			Map<String, Integer> shortNames = getDisplayNamesImpl(field, SHORT,
					locale);
			Map<String, Integer> longNames = getDisplayNamesImpl(field, LONG,
					locale);
			if (shortNames == null) {
				return longNames;
			}
	...
	/**
	 * Returns the maximum value that this field could have, given the current
	 * date. For example, with the date "Feb 3, 2540" and the
	 * <code>DAY_OF_MONTH</code> field, the actual maximum is 28; for "Feb 3,
	 * 2539" it is 29.
	 * 
	 * @param field

...
/*
 * @(#)src/classes/sov/java/text/BreakIterator.java, i18n, asdev, 20070119 1.16
 * ===========================================================================
 ...
 * ------  ----    ------  ----------------------------------------------------
 * 012713  110900  hdtdg:  Check for null locale in getBreakInstance()
 * 038807  261101  corbin  Change path for text resource bundle
 * 050596  230502  kwb     get resource in a doPriv
 * 78059   140904  corbin  Integrate 5.0 version
 */
...
public abstract class BreakIterator implements java.lang.Cloneable
{
    ...
    public Object clone()
    {
        try {
            ...
            throw new vjo.lang.InternalError();
        }
    }
    ...
    private static BreakIterator getBreakInstance(Locale where,
        ...
        Locale loc = where;                                         //ibm@12713

        if (loc == null) {                                          //ibm@12713
            loc = Locale.getDefault();                              //ibm@12713
        }                                                           //ibm@12713

        if (iterCache[type] != null) {
            BreakIteratorCache cache = (BreakIteratorCache) iterCache[type].get();
            if (cache != null) {
                if (cache.getLocale().equals(loc)) {                //ibm@12713
                    return cache.createBreakInstance();
                }
            }
        }

        BreakIterator result = createBreakInstance(loc,             //ibm@12713
                                                   type,
                                                   dataName,
                                                   dictionaryName);
        BreakIteratorCache cache = new BreakIteratorCache(loc, result);   //ibm@12713
    ...
    private static BreakIterator createBreakInstance(Locale where,
        ...
        ResourceBundle bundle = getBundle(                           //ibm.50596
        ...
        if (classNames[type].equals("RuleBasedBreakIterator")) {
            ...
            throw new vjo.lang.IllegalArgumentException("Invalid break iterator class \"" +
                            classNames[type] + "\"");
    }
        }
        catch (Exception e) {
            throw new vjo.lang.InternalError(e.toString()); 
        }
    }

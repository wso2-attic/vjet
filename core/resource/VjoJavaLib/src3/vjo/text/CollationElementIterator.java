...
/*
 * @(#)src/classes/sov/java/text/CollationElementIterator.java, i18n, asdev, 20070119 1.19
 * ===========================================================================
 ...
 * ------  ----   ------    --------------------------------------------------
 *  61476  040603 corbin    Add check to thai processing for null characters
 */
...
import vjo.java.sun.text.Normalizer;
import vjo.java.sun.text.NormalizerUtilities;
...
public final class CollationElementIterator
{
    ...
    public int next()
    {
        ...
        if (value == RuleBasedCollator.UNMAPPED) {
            ownBuffer[0] = UNMAPPEDCHARVALUE;                       //ibm.597
            ownBuffer[1] = ch << 16;                                //ibm.597
            buffer = ownBuffer;                                     //ibm.597
        }
        ...
        if (ordering.isSEAsianSwapping()) {
            ...
            if (isThaiPreVowel(ch)) {
                consonant = text.next();
		if (consonant == Normalizer.DONE) {                 //ibm.61476
		    return NULLORDER;				    //ibm.61476
		}						    //ibm.61476
        ...
        if (buffer != null) {                                       //ibm.597
            expIndex = 1;                                           //ibm.597
            value = buffer[0];                                      //ibm.597
        }
    ...
    public int previous()
    {
        ...
        if (value == RuleBasedCollator.UNMAPPED) {
            ...
            if (value >= RuleBasedCollator.CONTRACTCHARINDEX) {     //ibm.597
                value = prevContractChar(ch);                       //ibm.597
            }                                                       //ibm.597
            if (value >= RuleBasedCollator.EXPANDCHARINDEX) {       //ibm.597
                buffer = ordering.getExpandValueList(value);        //ibm.597
            }                                                       //ibm.597

            if (ordering.isSEAsianSwapping()) {
                char vowel = (char)text.previous();                           //ibm.597
                if (isThaiBaseConsonant(ch)) {                          //ibm.597

                    if (isThaiPreVowel(vowel)) {                        //ibm.597
                        buffer = makeReorderedBuffer(vowel, value, buffer, false); //ibm.597
                    }                                                   //ibm.597
                    else {                                              //ibm.597
                        text.next();                                    //ibm.597
                    }                                                   //ibm.597
        ...
        if (buffer != null) {                                       //ibm.597
            expIndex = buffer.length-1;                             //ibm.597
            value = buffer[expIndex];                               //ibm.597
        }                                                           //ibm.597
    ...
    private int[] makeReorderedBuffer(int colFirst,
        ...
        if (firstExpansion == null && lastExpansion == null) {
            ...
            if (firstExpansion == null) {
            	...
            	vjo.lang.System.arraycopy(firstExpansion, 0, result, 0, firstLength);
            }
            ...
            if (lastExpansion == null) {
                ...
                vjo.lang.System.arraycopy(lastExpansion, 0, result, firstLength, lastLength);
            }
        }
    ...
    private int[] ownBuffer = new int[2];                           //ibm.597
    private int expIndex = 0;
    private vjo.lang.StringBuffer key = new vjo.lang.StringBuffer(5);

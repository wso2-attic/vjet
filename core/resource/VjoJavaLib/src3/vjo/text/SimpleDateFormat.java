...
/*
 * @(#)src/classes/sov/java/text/SimpleDateFormat.java, i18n, asdev, 20070119 1.27
 * ===========================================================================
 ...
 * ------  ----   ------  ----------------------------------------------------
 * 007438  210200 hdbsw   Milliseconds format changed - see sun defect 4250683
 * 007856  020300 hdpgr   Merge to 1.3(t)
 * 023284  082800 katt    time off by an hour on OS/2 during DST
 * 033209  060701 ktsui   parse fails in Arabic
 * 034112  130701 ktsui   JCK: java_util.TimeZone has NullPointerException
 * 036651  041001 kwb     Allow alternate month names
 * 036967  161001 kwb     Fix parsing of alternate month names
 * 064296  150903 corbin  Integrate 1.4.2 changes
 * 078059  140904 corbin  Integrate 5.0 changes
 * 81658.1 060105 smithwilSimpleDateFromat does not parse colon(:) specified 
 *                        before mm
 * 104570.1 130606 corbin Completion of Date parsing Fix for Arabic Locale in 142
 */
...
import vjo.java.lang.Number ;
import vjo.java.lang.StringBuffer ;
import vjo.java.lang.StringBuilder ;

import vjo.java.util.TimeZone;
import vjo.java.util.Calendar;
import vjo.java.util.Date;
import vjo.java.util.Locale;
import vjo.java.util.ResourceBundle;
import vjo.java.util.SimpleTimeZone;
import vjo.java.util.Hashtable;
import vjo.java.util.GregorianCalendar;

import vjo.java.io.ObjectInputStream;

import vjo.java.sun.text.resources.LocaleData;
import vjo.java.sun.util.calendar.CalendarUtils;
import vjo.java.sun.util.calendar.ZoneInfoFile;
...
public class SimpleDateFormat extends DateFormat {
    ...
    private static boolean ambiguousNegative=false;     //ibm@33209
    ...
    private void initialize(Locale loc) {
        ...
        if (pattern != null && pattern.indexOf('-') > 0) //ibm@33209 //ibm@34112
            try {                               //ibm@33209
                if (numberFormat.parse(new String("1-")).intValue() == -1) //ibm@33209
                    ambiguousNegative = true;   //ibm@33209
            }                                   //ibm@33209
        catch (ParseException e) {}             //ibm@33209
    }
	...
	// zeroPaddingNumber() must be fixed.
		...
		zeroPaddingNumber(value, 2, 2, buffer); // clip 1996 to 96
            break;
        case 2: // 'M' - MONTH
            // Allow for alternate month names (ie. subjective case)
            if (count >= 4) {
                if (count >= 5 && formatData.months.length>=26) {  //ibm.36651
                    current = formatData.months[value+13];         //ibm.36651
                } else {                                           //ibm.36651
...
//Begin of ibm@104570.1
                if (ambiguousNegative){
                    if (start+count < text.length()){
                        if (text.charAt(start+count) == '-'){
                            //This could be a negative number or dash
                            obeyCount = true;
                            if (start+count+1 < text.length()){
                                if (text.charAt(start+count+1) == '-'){
                                    //This handles the case where "--" is in pattern
                                    count++;
                                }
                            }
                        }
...
//End of ibm@104570.1
    ...
    private int matchString(String text, int start, int field, String[] data)
    {
        ...
        if (bestMatch >= 0)
        {
            if (field==Calendar.MONTH)         //ibm.36967
                bestMatch %= 13;               //ibm.36967

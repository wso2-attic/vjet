...
/*
 * @(#)src/classes/sov/java/net/InetAddress.java, net, asdev, 20070119 1.21
 * ===========================================================================
 ...
 * ------  ------  ------   --------------------------------------------------
 * 081637  100105  stalleyj Apply PCR 1555 - Cache local host
...
import vjo.lang.Character ;
import vjo.lang.SecurityManager ;
import vjo.lang.System ;

import vjo.util.HashMap;
import vjo.util.LinkedHashMap;
import vjo.util.Random;
import vjo.util.LinkedList;

import vjo.security.AccessController;

import vjo.sun.security.action.GetBooleanAction;
import vjo.sun.security.action.GetPropertyAction;
import vjo.sun.security.action.LoadLibraryAction;

import vjo.sun.net.InetAddressCachePolicy;
import vjo.sun.net.util.IPAddressUtil;

import vjo.sun.misc.Service;

import vjo.sun.net.spi.nameservice.*;
...
class InetAddress implements java.io.Serializable {
    ...
    // ibm@81637 start
    private static String localHostName = null; // synchronize on lock below when accessing
    private static Object localHostNameLock = new Object();
    private static boolean cacheLocalHost = false;

    static {
        cacheLocalHost
            = ((Boolean)java.security.AccessController.doPrivileged(
                new GetBooleanAction("com.ibm.cacheLocalHost"))).booleanValue();
    }
    // ibm@81637 end
	...
	int n = 1;
		...
		if (provider.equals("default")) {
			...
			public InetAddress[] lookupAllHostAddr(String host) /*ibm@99499*/
	...
	if ((obj = checkLookupTable(host)) == null) {
		...
		obj = nameService.lookupAllHostAddr(host); /*ibm@99499*/
		success = true;
	    } catch (UnknownHostException uhe) {
		obj  = unknown_array; 
		success = false;
	...
	try {
            ...
            synchronized(localHostNameLock) {
                if (cacheLocalHost && localHostName != null) {
                    localAddrs = (InetAddress[]) getCachedAddress(localHostName);
                    if (localAddrs != null) {
                        return localAddrs[0];
                    }
                }

                // Either we don't know the name for the localHost or the
                // existing cache entry has expired, in which case we need
                // to get the name directly in case it has changed.

                localHostName = impl.getLocalHostName();

                if (security != null) {
                    security.checkConnect(localHostName, -1);
                }
                // we are calling getAddressFromNameService directly
                // to avoid getting localHost from cache

                try {
                    localAddrs = (InetAddress[]) 
                        InetAddress.getAddressFromNameService(localHostName);
                } catch (UnknownHostException uhe) {
                    throw new UnknownHostException(localHostName + ": " + uhe.getMessage());
                }
            }
            // ibm@81637 end 

...
//			|| VjoConstants.NativeTypes.getNumberJstType().equals(rhs);
			...
			|| VjoConstants.NativeTypes.getPrimitiveBooleanJstType().equals(rhs)
	...
	protected static boolean isAssignableVjoType(IJstType lhs, IJstType rhs){
		...
		for(IJstType inheritedType: rhs.getExtends()){
			if (equals(VjoConstants.NativeTypes.getObjectJstType(), inheritedType)) {
				continue;
			}
...
//		}
	  //Added by Eric.Ma 20100416
		...
		if(one instanceof JstTypeWithArgs){
			if(two instanceof JstTypeWithArgs){
				...
				return isAssignableVjoTypeWithArgs(null, null, (JstTypeWithArgs)one, (JstTypeWithArgs)two);
			}
			else{
				//one is generics
		...
		if(isAssignable(one.getType(),two.getType())) {
			...
			if(oneArgTypes.size() == twoArgTypes.size()){
				for(int i = 0, len = oneArgTypes.size(); i < len; i++){
					...
					if (oneArgType instanceof JstTypeWithArgs && twoArgType instanceof JstTypeWithArgs) {
						if (!isAssignableVjoTypeWithArgs(typeWithArgsOne, typeWithArgsTwo, (JstTypeWithArgs)oneArgType, (JstTypeWithArgs)twoArgType)) {
							return false;
						}
						else {
							continue;
                    ...
                    if (oneArgType instanceof JstWildcardType
                        ...
                        if (isSuperType(((JstWildcardType) twoArgType)
                                .isUpperBound(), ((JstWildcardType) oneArgType)
                                .isUpperBound(), getWildCardType((JstWildcardType)twoArgType),
                    ...
                    if (oneArgType instanceof JstParamType) {
                        if (isSuperType(twoArgType.getOwnerType(), oneArgType
                                .getOwnerType())) {
					...
					if (oneArgType2 instanceof JstParamType && typeWithArgsOne != null) {
						IJstType argType = typeWithArgsOne.getParamArgType((JstParamType)oneArgType2);
					...
					if (twoArgType2 instanceof JstParamType && typeWithArgsTwo != null) {
						IJstType argType = typeWithArgsTwo.getParamArgType((JstParamType)twoArgType2);
					...
					//TODO allow boundary
					if(oneArgType2 != null 
							&& (((JstWildcardType)oneArgType).isUpperBound() && isAssignable(twoArgType2, oneArgType2)
									|| (((JstWildcardType)oneArgType).isLowerBound() && isAssignable(oneArgType2, twoArgType2)))){
						continue;
					}
					else if(JstWildcardType.class.isAssignableFrom(oneArgType.getClass())){
						final JstWildcardType wildCardType = ((JstWildcardType)oneArgType);
						final IJstType assignableType = JstWildcardType.class.isAssignableFrom(twoArgType.getClass())
							? ((JstWildcardType)twoArgType).getType() : twoArgType;
							
						if(wildCardType.isLowerBound()){
							if(!isAssignable(wildCardType.getType(), assignableType)){
								return false;
							}
						}
						else if(wildCardType.isUpperBound()){
							if(!isAssignable(assignableType, wildCardType.getType())){
								return false;
							}
						}
						else{
	...
	public static IJstType assign(final IJstType assignTo, final IJstType assignFrom){
		final IJstType assignToTransformed = toJsNativeType(assignTo);
		if(assignToTransformed != null){
			if(VjoConstants.NativeTypes.getStringJstType().equals(assignToTransformed)){
	...
	public static boolean isAssignable(List<IJstType> assignToList, List<IJstType> assignFromList){
		if(assignToList == null
			...
			return false;//TODO should probably be an exception, IllegalArgumentException etc.
		}
	...
	public static boolean isAssignable(JstTypeWithArgs typeWithArgsTo, JstTypeWithArgs typeWithArgsFrom, JstTypeWithArgs assignTo, JstTypeWithArgs assignFrom) {
		...
		return isAssignableVjoTypeWithArgs(typeWithArgsTo, typeWithArgsFrom, assignTo, assignFrom);
	}
	
	public static boolean isAssignable(IJstType assignTo, IJstType assignFrom){
		if (assignTo instanceof IInferred ||
			assignFrom instanceof IInferred) {
		...
		if (assignFrom instanceof IJstRefType) {
			//added by huzhou@ebay.com, any type reference should be assignable to Function
		...
		if(!(fromType instanceof SynthOlType) && equals(VjoConstants.NativeTypes.getObjectJstType(), fromType)){
			...
			return isAssignableJsNativeType(assignToTransformed, assignFromTransformed);
		}
		else{
	...
	/**
	 * added by huzhou to handle obj literal assignments
	 * ObjLiteral type is different from Object type as the value must be an Object Literal declaration or equivalent
	 * Therefore an ObjLiteral value could only be assigned to Object (handled by caller)
	 * or ObjLiteral
	 * or Otype
	 * @param assignTo
	 * @param assignFrom
	...
	private static Boolean checkObjLiteral(IJstType assignTo, IJstType assignFrom) {
		if((assignTo instanceof IJstType 
				&& "ObjLiteral".equals(assignTo.getSimpleName()))//raw ObjLiteral type
			||(assignTo instanceof SynthOlType
					&& ((SynthOlType)assignTo).getResolvedOType() == null)){//unresolved SynthOlType
			return (assignFrom instanceof IJstType && "ObjLiteral".equals(assignFrom.getSimpleName()))
				|| assignFrom instanceof SynthOlType
				|| assignFrom instanceof JstObjectLiteralType
				? Boolean.TRUE : Boolean.FALSE;
		}
		else if(assignTo instanceof JstObjectLiteralType){
			if(assignFrom instanceof JstObjectLiteralType){
				return isAssignable((JstObjectLiteralType)assignTo, (JstObjectLiteralType)assignFrom)
					? Boolean.TRUE : Boolean.FALSE;
			}
			else if(assignFrom instanceof SynthOlType){
				if(((SynthOlType)assignFrom).getResolvedOType() != null){
					return isAssignable((JstObjectLiteralType)assignTo, (JstObjectLiteralType)((SynthOlType)assignFrom).getResolvedOType())
					? Boolean.TRUE : Boolean.FALSE;
				}
				else{//when SynthOlType isn't bound to otype, it should also match the lhs's properties declarations
					//it's not enforced because there's an otype declaration case, where otype defines JstObjectLiteralType member
					//and its property expression is obj literal with SynthOlType
					//but the SynthOlType doesn't have any properties or members at all
					//seems that the otype translation doesn't care the expression value translation at all
					return Boolean.TRUE;
				}
			}
		}
	...
	private static boolean isAssignable(final JstObjectLiteralType lhs, 
		...
		for(IJstProperty lhsPty : lhs.getProperties()){
			final IJstProperty rhsPty = rhs.getProperty(lhsPty.getName().getName());
			if(rhsPty != null && !isAssignable(lhsPty.getType(), rhsPty.getType())){
				return false;
			}
	...
	private static Boolean checkFunction(IJstType assignTo, IJstType assignFrom) {
		if(assignTo instanceof JstFuncType
			...
			return Boolean.valueOf(isAssignable(((JstFuncType)assignTo).getFunction(), ((JstFuncType)assignFrom).getFunction()));
		}
		
		if(assignTo instanceof JstFunctionRefType
				&& assignFrom instanceof JstFunctionRefType) {
			return Boolean.valueOf(isAssignable(((JstFunctionRefType)assignTo).getMethodRef(), ((JstFunctionRefType)assignFrom).getMethodRef()));
		}
		
		if(assignTo instanceof JstFuncType
				&& assignFrom instanceof JstFunctionRefType) {
			return Boolean.valueOf(isAssignable(((JstFuncType)assignTo).getFunction(), ((JstFunctionRefType)assignFrom).getMethodRef()));
		}
		
		if(assignTo instanceof JstFunctionRefType
				&& assignFrom instanceof JstFuncType) {
			return Boolean.valueOf(isAssignable(((JstFunctionRefType)assignTo).getMethodRef(), ((JstFuncType)assignFrom).getFunction()));
		}
		...
		if(assignTo instanceof JstFuncType
			...
			return Boolean.valueOf(isAssignable(((JstFuncType)assignTo).getFunction(), invoke));
		}
		...
		if(assignTo instanceof JstFunctionRefType
			...
			return Boolean.valueOf(isAssignable(((JstFunctionRefType)assignTo).getMethodRef(), invoke));
		}
		
		return null;
	}
	
	public static boolean isAssignable(final IJstMethod lhsMethod, final IJstMethod rhsMethod){
		...
		NEXT_LHS:
		for(IJstMethod lhsIter : lhsMethods){
	...
	private static boolean isAssignableNoOverload(final IJstMethod lhsMethod, final IJstMethod rhsMethod){
		...
		if(lhsRtnType != null && rhsRtnType != null){
			if(!"void".equals(lhsRtnType.getName())){//if lhs is void, rhs's rtn type won't impact the assignabilities
				if(!isAssignable(lhsRtnType, rhsRtnType)){ //return type of rhs should be more restrict
					return false;
				}
			}
		}
		...
		final List<JstArg> rhsParams = paddingRhsParams(rhsMethod, lhsParamsLength);
		...
		for(Iterator<JstArg> lhsIt = lhsParams.iterator(), rhsIt = rhsParams.iterator(); lhsIt.hasNext() && rhsIt.hasNext();){
			...
			//fix by huzhou to tell the difference between variable lengthed parameters
			//if lhs is variable lengthed while rhs isn't, it won't match
			if(!isAssignable(rhsParam.getType(), lhsParam.getType())){ //parameter type of rhs should be less restrict
				return false;
			}
		}
	...
	/**
	 * helper for isAssignableNoOverload, which only pads the rhs's params list when:
	 * 1. lhs's params list is longer than rhs'
	 * 2. rhs's last param is a variable lengthed param
	 * 3. the padding result will make the rhs' params list as long as the lhs'
	 * @param rhsMethod
	...
	private static List<JstArg> paddingRhsParams(final IJstMethod rhsMethod,
		...
		final int rhsParamsLength = rhsParams.size();
		final JstArg lastRhsParam = rhsParamsLength > 0 ? rhsParams.get(rhsParamsLength - 1) : null;
		...
		if(lhsParamsLength > rhsParamsLength/* && rhsParamsLength > 0*/){
			if(lastRhsParam != null && lastRhsParam.isVariable()){
				final List<JstArg> paddingRhsParams = new LinkedList<JstArg>(rhsParams);
				...
				for(int i = rhsParamsLength; i < lhsParamsLength; i++){
					...
					paddingRhsParams.add(new JstArg(JstCache.getInstance().getType("Object"), "proxy", true));
				}
				return paddingRhsParams;
			}
		}
		//bugfix by huzhou@ebay.com
		//when lhs's param length is one smaller than rhs's, we further check if rhs' last param is variable lengthed
		//if both condition met, we use the rhs's parameters excluding the last variable lengthed one
		else if(lhsParamsLength == rhsParamsLength - 1 && lastRhsParam.isVariable()){
	...
	public static boolean isCastable(IJstType origin, IJstType castTo){
		...
		if(originTransformed != null){
			return isAssignableJsNativeType(originTransformed, toJsNativeType(castTo));
		}
		else{
	...
	public static boolean isBoolean(IJstType exprValue){
		...
		if(exprValue != null){
			if(!VjoConstants.NativeTypes.getPrimitiveBooleanJstType().equals(toJsNativeType(exprValue))){
				return false;
			}
		}
	...
	public static boolean isObject(IJstType exprValue){
		...
		if(exprValue != null){
			if(!equals(VjoConstants.NativeTypes.getObjectJstType(), toJsNativeType(exprValue))){
				return false;
			}
		}
...
//		
	...
	public static boolean isNumber(IJstType exprValue){
		...
		if(exprValue != null){
			if(!equals(VjoConstants.NativeTypes.getNumberJstType(), toJsNativeType(exprValue))){
				return false;
			}
		}
...
//	}
	...
	public static boolean isString(IJstType exprValue){
		...
		if(exprValue != null){
			if(!equals(VjoConstants.NativeTypes.getStringJstType(), toJsNativeType(exprValue))){
				return false;
			}
		}
    ...
    public static IJstType getAttributedType(final JstAttributedType attributedType){

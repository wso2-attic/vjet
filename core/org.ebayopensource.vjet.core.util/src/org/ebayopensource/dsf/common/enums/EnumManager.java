...
public class EnumManager {
	...
	private static final ConcurrentMap<Class,EnumManager> s_managers = 
			new ConcurrentHashMap<Class,EnumManager>();
	 ...
	 *
	 * @param enum an Enum object to be added into the collection.
	 */
	...
	static Object createEnclosedEnumArray(Class clz, Class[] enclosingClzz) {
		...
		for (int i = 0; i < enclosingClzz.length; i++) {
			...
			if (!clz.isAssignableFrom(childClz)) {
				throw new IllegalArgumentException("createEnclosingEnumArray: " + childClz + " is not derived from "
					+ clz);
			}
	...
	static Object createInheritedEnumArray(Class clz, Class topClz) {
		...
		if (!topClz.isAssignableFrom(clz)) {
			throw new IllegalArgumentException("createInheriedEnumArray: " + topClz + " is not the base type for "
				+ clz);
		}
		EnumManager theManager = getManager(clz);
		List allEnums = new ArrayList();

package org.ebayopensource.dsf.logger;
  ...
  import com.ebay.kernel.context.LoggingContext;
  import com.ebay.kernel.context.LoggingContext.ContextLoggingOption;
  ...
  import com.ebay.kernel.logger.rt.ThrottlingManager;
  import com.ebay.kernel.message.MessageFormatter;
  import com.ebay.kernel.message.MessageFormatterFactory;
  import com.ebay.kernel.resource.ResourceUtil;
  
  /**
   * Logger is a class that logs messages. It calls JDK 1.5 logger 
   * (java.util.logging.Logger) in to actually perform the logging.
   * This class should be a very thin proxy to the jdk logger. There
   * are some methods not really thin, but they are just left for
  ...
  public class Logger {
      ...
      private static final ConcurrentMap<String,Logger> s_loggerList = 
              new ConcurrentHashMap<String, Logger>(25);
  
      // Formatter for com.ebay.kernel.message.Message objects
      private static final MessageFormatter s_messageFormatter = 
              MessageFormatterFactory.getInstance().getMessageFormatter();
       ...
       *
       * @param Class callingClass - The class that invoked the logger.
       */
       ...
       *
       * @param subsystem The name of the subsystem for which the
       * logger is being instantiated.
       */
      ...
      /**
       * The constructor is private because we don't want the developer
       * creating new logger instances for the same subsystem. Instead,
       * an instance can be accessed using the getInstance() method.
       *
       * @param String subsystem - The class that invoked the logger.
       */
      private Logger(String subsystem) {
          ...
          try {
              ...
              System.err.println(this.getClass().getName() + " : init error - " + e.getMessage()); //KEEPME
      ...
      protected static synchronized void initConfig() throws IOException
      {
          ...
          //JDK logger uses two system properties "java.util.logging.config.class"
          ...
          // If above two system properties are not defined, kernel logger will find 
          ...
          Collection<String> ebayHandlerNames) throws IOException
      {
          ...
          for (Map.Entry<Object,Object> e: props.entrySet()) {
              ...
              name = name.replaceAll("com.ebay.logger.", "com.ebay.kernel.logger.rt.");
              value = value.replaceAll("com.ebay.logger.", "com.ebay.kernel.logger.rt.");
      ...
      /**
       * Log the given message at a given level: dhe added for log English msg, such as system error msg
       *
       * @param int logLevel - From com.ebay.LogLevel.
       * @param String message - Given message to log.
       */
      ...
      public void log(LogLevel logLevel, String message, Object[] messageArgs, boolean includeContextInfo) {
          ...
          try {
              LoggingContext.setContextLoggingOption(includeContextInfo? ContextLoggingOption.ON: ContextLoggingOption.OFF);
              m_logger.log(logLevel.getLevel(), message, messageArgs);
          } finally {
      ...
      /**
       * Support 'Turn off context logging'. This method can be used when Context contains sensitive parameters that should not be logged as is to CAL.
       ...
       * @param int logLevel - From com.ebay.LogLevel.
       * @param String message - Given message to log.
       * @param boolean includeContextInfo -
       *          Indicate whether context info should be part of the log or not
       */
       ...
       *
       * @param int logLevel - From com.ebay.LogLevel.
       * @param Throwable t - Given exception.
       */
       ...
       * instance of RuntimeException or Error. Otherwise, logs with LogLevel.ERROR.
      ...
      protected synchronized static void enableTraceThrottle(boolean value) {
          ThrottlingManager.getInstance().setLogBufferEnabled(value);
      }
  }
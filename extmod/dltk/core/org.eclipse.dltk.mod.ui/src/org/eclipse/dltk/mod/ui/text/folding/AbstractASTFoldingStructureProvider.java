...
public abstract class AbstractASTFoldingStructureProvider implements
	 ...
	 * addProjectionRange}.
	 */
	public class FoldingStructureComputationContext {
	...
	protected IModelElement fInput;
	...
	protected IModelElement getInputElement() {
		if (fEditor == null)
			return null;
	...
	private boolean isCommentType(String type) {
		String[] commentTypes = getCommentPartition();
		for (int i = 0; i < commentTypes.length; i++) {
			String commentType = commentTypes[i];
			if (type.equals(commentType)) {
				return true;
			}

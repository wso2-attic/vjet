...
public abstract class AbstractConfigurationBlock implements
	...
	private org.eclipse.dltk.mod.internal.ui.dialogs.StatusInfo fStatus;
		...
		while (iter2.hasNext()) {
			...
			for (Iterator iterator = data.keySet().iterator(); iterator
				...
				if (data.get(title).equals(value)) {
					...
					// When this page is open, it will initialize the
					// 'indentation size' textbox.
					if (title.equals("Tabs only")) {
						isForTabOnly = true;
					}
					break;
				}

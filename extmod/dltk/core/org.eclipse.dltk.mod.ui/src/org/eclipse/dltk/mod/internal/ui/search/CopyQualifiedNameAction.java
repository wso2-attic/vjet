...
import org.eclipse.dltk.mod.internal.corext.refactoring.reorg.IReorgEnablementPolicy;
...
public class CopyQualifiedNameAction extends SelectionDispatchAction {
	...
	public CopyQualifiedNameAction(IWorkbenchSite site) {
		super(site);
		// ActionMessages.CopyQualifiedNameAction_ActionName
		setText(SearchMessages.SearchPage_CopyQualifiedNameAction_ActionName);
		setImageDescriptor(DLTKPluginImages.DESC_ELCL_COPY_QUALIFIED_NAME);
	...
	private static class ClipboardCopier {
		...
		private String getName(IModelElement modelElement) {
			if (modelElement instanceof JSSourceType) {
				String fullQualifiedName = ((JSSourceType) modelElement)
	...
	private static class CopyToClipboardEnablementPolicy implements
		...
		private boolean canCopyAllToClipboard() throws ModelException {
			...
			for (int i = 0; i < fScriptElements.length; i++) {
				if (!canCopyToClipboard(fScriptElements[i]))
					return false;
			}
			return true;
		}
		...
		private static boolean canCopyToClipboard(IModelElement element)
			...
			// TODO: Add copy operation for external script folders.. Posible
		...
		private boolean hasProjects() {
			...
			for (int i = 0; i < fScriptElements.length; i++) {
				if (ReorgUtils.isProject(fScriptElements[i]))
					return true;
			}
			return false;
		}

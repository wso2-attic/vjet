...
/**
 * EBAY MOD Extract from
 * org.eclipse.jdt.internal.ui.workingsets.SimpleWorkingSetSelectionDialog
 * (eclipse 3.3) for supporting eclipse 3.5
 */
public class SimpleWorkingSetSelectionDialog extends SelectionDialog {
	...
	/**
	 * add by patrick for support eclipse 3.5, extract the constant value from
	 * org.eclipse.jdt.internal.ui.workingsets.JavaWorkingSetUpdater
	 */
	static final String JavaWorkingSetUpdater_ID = "org.eclipse.jdt.ui.JavaWorkingSetPage";
	...
	public SimpleWorkingSetSelectionDialog(Shell shell, String[] workingSetIds,
			IWorkingSet[] initialSelection) {
		super(shell);

		setTitle(Messages.SimpleWorkingSetSelectionDialog_SimpleSelectWorkingSetDialog_title);
		setHelpAvailable(false);
		setShellStyle(getShellStyle() | SWT.RESIZE);

		fWorkingSets = WorkingSetConfigurationBlock.filter(PlatformUI
				.getWorkbench().getWorkingSetManager().getWorkingSets(),
				workingSetIds);
	...
	protected CheckboxTableViewer createTableViewer(Composite parent) {
		CheckboxTableViewer result = CheckboxTableViewer.newCheckList(parent,
	...
	protected ViewerSorter createTableSorter() {
		return new ViewerSorter() {
			public int compare(Viewer viewer, Object e1, Object e2) {
				...
				return Collator.getInstance().compare(w1.getLabel(),
						w2.getLabel());
			}
		};
	}
	...
	protected void createButtonsForRightButtonBar(Composite bar) {
		...
		fSelectAll
				.setText(Messages.SimpleWorkingSetSelectionDialog_SelectAll_button);
		...
		fDeselectAll
				.setText(Messages.SimpleWorkingSetSelectionDialog_DeselectAll_button);
		...
		fNewWorkingSet
				.setText(Messages.SimpleWorkingSetSelectionDialog_New_button);
	...
	protected void checkedStateChanged() {
		...
		fCheckedElements = (IWorkingSet[]) elements
				.toArray(new IWorkingSet[elements.size()]);
	}
	...
	protected IWorkingSet newWorkingSet() {
		...
		// can only allow to create java working sets at the moment, see bug
		// 186762
		// IWorkingSetNewWizard wizard=
		// manager.createWorkingSetNewWizard(fWorkingSetIds);
		// if (wizard == null)
		// return;

		IWorkingSetNewWizard wizard = manager
				.createWorkingSetNewWizard(new String[] { JavaWorkingSetUpdater_ID });

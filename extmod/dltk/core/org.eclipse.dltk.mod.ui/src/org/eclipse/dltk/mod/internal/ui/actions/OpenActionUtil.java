...
public class OpenActionUtil {
	...
	/*
	 * rewrite the logic by xingzhu: if 'element' is a source reference element,
	 * directly open the corresponding resource
	 * 
	 * Opens the editor on the given element and subsequently selects it.
	 */
	public static void open(Object element, boolean activate)
		...
		if (element instanceof SourceRefElement
				&& ((SourceRefElement) element).getResource() != null
				&& ((SourceRefElement) element).getResource().exists())
			part = EditorUtility.openInEditor(((SourceRefElement) element)
					.getResource());
		else
			part = EditorUtility.openInEditor(element, activate);

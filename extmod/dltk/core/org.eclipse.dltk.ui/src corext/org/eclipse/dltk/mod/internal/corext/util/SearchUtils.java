...
public class SearchUtils {
	...
	/**
	 * @param match
	 * @return the enclosing {@link ISourceModule} of the given match, or null
	 *         iff none
	 */
	public static ISourceModule getSourceModule(SearchMatch match) {
		...
		if (enclosingElement != null) {
			...
			ISourceModule cu = (ISourceModule) enclosingElement
		...
		if (jElement != null && jElement.exists()
				&& jElement.getElementType() == IModelElement.SOURCE_MODULE)
			return (ISourceModule) jElement;
		return null;
	}

	public static SearchParticipant[] getDefaultSearchParticipants() {
		return new SearchParticipant[] { SearchEngine
				.getDefaultSearchParticipant() };
	}

	/**
	 * Constant for use as matchRule in
	 * {@link SearchPattern#createPattern(IModelElement, int, int)} to get
	 * search behavior as of 3.1M3 (all generic instantiations are found).
	 */

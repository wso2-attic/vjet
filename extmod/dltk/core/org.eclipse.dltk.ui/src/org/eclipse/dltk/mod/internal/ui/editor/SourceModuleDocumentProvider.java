...
import org.eclipse.dltk.mod.ui.editor.SourceModuleAnnotationModelEvent;
...
public class SourceModuleDocumentProvider extends TextFileDocumentProvider
	...
	protected static class SourceModuleAnnotationModel extends
		...
		protected MarkerAnnotation createMarkerAnnotation(IMarker marker) {
			...
			if (markerType != null
				...
				if (markerType.equals(DefaultProblem.MARKER_TYPE_TASK)) {
					return null;
				} else {
	...
	protected static class ExternalSourceModuleAnnotationModel extends
			...
			for (int i = 0; i < markers.length; i++) {
				...
				try {
					if (markerLocation != null
							&& isSameLocation(moduleLocation, markerLocation)) {
		...
		// EBAY MOD START
		private boolean isSameLocation(String moduleLocation,
				String markerLocation) throws URISyntaxException {
			if (moduleLocation.equals(markerLocation)) {
				return true;
			}

			URI moduleURI = convert2URI(moduleLocation);
			URI markerURI = convert2URI(markerLocation);

			return moduleURI.equals(markerURI);
		}

		private URI convert2URI(String moduleLocation)
				throws URISyntaxException {
			IPath modulePath = Path.fromPortableString(moduleLocation);
			if (EnvironmentPathUtils.isFull(modulePath)) {
				modulePath = EnvironmentPathUtils.getLocalPath(modulePath);
				if (modulePath.toString().contains("jar!")) {
					moduleLocation = "jar:file:/" + modulePath.toString();
				} else {
					moduleLocation = "file:/" + modulePath.toString();
				}
			} else {
				moduleLocation = modulePath.toString();
			}
			return new URI(moduleLocation);
		}
	...
	protected FileInfo createFileInfo(Object element) throws CoreException {
		...
		// EBAY MOD START
		if (info.fModel == null || (original instanceof IExternalSourceModule)) {
			// EBAN MOD END
	...
	public void connect(Object element) throws CoreException {
		...
		if (info == null) {
			...
			if (element instanceof IStorageEditorInput) {
				IStorageEditorInput storageEditorInput = (IStorageEditorInput) element;
				path = storageEditorInput.getStorage().getFullPath();
				IModelElement e = (IModelElement) storageEditorInput
						.getAdapter(IModelElement.class);
				if (e != null && e instanceof ISourceModule) {
					cu = (ISourceModule) e;
				}
			} else if (element instanceof ExternalFileEditorInput) {
				ExternalFileEditorInput efei = (ExternalFileEditorInput) element;
				path = efei.getStorage().getFullPath();
				IModelElement e = efei.getModelElement();
				if (e != null && e instanceof ISourceModule) {
					cu = (ISourceModule) e;
				}
			}
			ExternalSourceModuleAnnotationModel annotationModel = null;
			if (cu == null) {
				cu = createFakeSourceModule(element, true);
				annotationModel = new ExternalSourceModuleAnnotationModel(path);
			}
			if (cu == null)
				return;
			...
			info.fModel = annotationModel != null ? annotationModel

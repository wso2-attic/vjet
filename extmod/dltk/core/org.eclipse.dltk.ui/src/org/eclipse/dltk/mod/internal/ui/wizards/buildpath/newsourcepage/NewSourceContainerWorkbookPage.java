...
import org.eclipse.dltk.mod.internal.corext.buildpath.BuildpathModifier.IBuildpathModifierListener;
import org.eclipse.dltk.mod.internal.ui.preferences.ScrolledPageContent;
import org.eclipse.dltk.mod.internal.ui.util.ViewerPane;
...
public class NewSourceContainerWorkbookPage extends BuildPathBasePage implements
		IBuildpathModifierListener {

	public static final String OPEN_SETTING = "org.eclipse.dltk.mod.internal.ui.wizards.buildpaths.NewSourceContainerPage.openSetting"; //$NON-NLS-1$
	...
	/**
	 * Constructor of the <code>NewSourceContainerWorkbookPage</code> which
	 ...
	 * @param context
	 *            a runnable context, can be <code>null</code>
	 */
	public NewSourceContainerWorkbookPage(ListDialogField BuildpathList,
			IRunnableContext context, IPreferenceStore store) {
	...
	public Control getControl(Composite parent) {
		...
		// ScrolledPageContent is needed for resizing on expand the expandable
		// composite
		...
		final ExpandableComposite excomposite = new ExpandableComposite(
				sashForm, SWT.NONE, ExpandableComposite.TWISTIE
						| ExpandableComposite.CLIENT_INDENT);
		excomposite.setFont(sashForm.getFont());
		excomposite
				.setText(NewWizardMessages.NewSourceContainerWorkbookPage_HintTextGroup_title);
		...
		excomposite.addExpansionListener(new ExpansionAdapter() {
			public void expansionStateChanged(ExpansionEvent e) {
				...
				if (parentScrolledComposite != null) {
					...
					// This line is commented out since it causes horizontal
					// scroll bar to appear
					// when collapsing/expanding HintTextGroup.
		...
		final DialogPackageExplorerActionGroup actionGroup = new DialogPackageExplorerActionGroup(
				fHintTextGroup, this);

		Composite outputLocation = new Composite(body, SWT.NONE);
		outputLocation.setLayout(new GridLayout(2, false));
		outputLocation.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false));

		LayoutUtil.doDefaultLayout(outputLocation, new DialogField[] {}, true,
	 ...
	 * @param sashForm
	 *            the sash form to apply the new weights to
	 * @param isExpanded
	 *            <code>true</code> if the expandable composite is expanded,
	...
	private void adjustSashForm(int[] sashWeight, SashForm sashForm,
		...
		if (isExpanded) {
			...
			// TODO Dividing by 10 because of
			// https://bugs.eclipse.org/bugs/show_bug.cgi?id=81939
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (obj instanceof IProjectFragment) {
					...
					BPListElement cpElement = BuildpathModifier
							.getBuildpathEntry(existingEntries, element);
					selectedList.add(cpElement);
				} else if (obj instanceof IScriptProject) {
					IBuildpathEntry entry = BuildpathModifier
							.getBuildpathEntryFor(project.getPath(), project,
									IBuildpathEntry.BPE_SOURCE);
					if (entry == null)
						continue;
					BPListElement cpElement = BPListElement.createFromExisting(
							entry, project);
					selectedList.add(cpElement);
				}
			}

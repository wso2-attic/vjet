 ...
 * org.eclipse.jdt.internal.ui.workingsets.WorkingSetConfigurationBlock(eclipse
 * 3.3) for supporting eclipse 3.5
 */
public class WorkingSetConfigurationBlock {
	 ...
	 */
	public static IWorkingSet[] getSelectedWorkingSet(
	 ...
	 */
	public static void addToWorkingSets(IAdaptable element,
	 ...
	 * @param enableButtonText
	 *            the text shown for the enable button
	 * @param settings
	 *            to store/load the selection history
	 */
	public WorkingSetConfigurationBlock(String[] workingSetIds,
			String enableButtonText, IDialogSettings settings) {
	...
	/**
	 * @param message
	 *            the message to show to the user in the working set selection
	 *            dialog
	 */
	...
	public void createContent(final Composite parent) {
		...
		fLabel.setText(Messages.WorkingSetConfigurationBlock_WorkingSetText_name);
		...
		fConfigure
				.setText(Messages.WorkingSetConfigurationBlock_SelectWorkingSet_button);
		GridData configureData = new GridData(SWT.LEFT, SWT.CENTER, false,
				false);
		...
		fConfigure.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				SimpleWorkingSetSelectionDialog dialog = new SimpleWorkingSetSelectionDialog(
						parent.getShell(), fWorkingSetIds, fSelectedWorkingSets);
				if (fMessage != null)
		...
		if (fSelectedWorkingSets.length == 0 && fSelectionHistory.size() > 0) {
			fWorkingSetCombo.select(historyIndex((String) fSelectionHistory
					.get(0)));
			updateSelectedWorkingSets();
		} else {
			updateWorkingSetSelection();
	...
	private void updateEnableState(boolean enabled) {
		...
		fWorkingSetCombo
				.setEnabled(enabled
						&& (fSelectedWorkingSets.length > 0 || getHistoryEntries().length > 0));
	...
	private String[] getHistoryEntries() {
		String[] history = (String[]) fSelectionHistory
				.toArray(new String[fSelectionHistory.size()]);
	...
	private void updateSelectedWorkingSets() {
		String item = fWorkingSetCombo.getItem(fWorkingSetCombo
				.getSelectionIndex());
	...
	private void storeSelectionHistory(IDialogSettings settings) {
		...
		if (fSelectionHistory.size() > MAX_HISTORY_SIZE) {
			...
			history = (String[]) fSelectionHistory
					.toArray(new String[fSelectionHistory.size()]);
		}

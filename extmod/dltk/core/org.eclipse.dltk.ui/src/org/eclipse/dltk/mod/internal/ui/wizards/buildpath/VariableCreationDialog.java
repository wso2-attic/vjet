...
public class VariableCreationDialog extends StatusDialog {
	...
	private BPVariableElement fElement;

	private List fExistingNames;

	public VariableCreationDialog(Shell parent, BPVariableElement element,
			List existingNames) {
		super(parent);
		if (element == null) {
		...
		fNameField
				.setLabelText(NewWizardMessages.VariableCreationDialog_name_label);

		fPathField = new StringButtonDialogField(adapter);
		fPathField.setDialogFieldListener(adapter);
		fPathField
				.setLabelText(NewWizardMessages.VariableCreationDialog_path_label);
		fPathField
				.setButtonLabel(NewWizardMessages.VariableCreationDialog_path_file_button);

		fDirButton = new SelectionButtonDialogField(SWT.PUSH);
		fDirButton.setDialogFieldListener(adapter);
		fDirButton
				.setLabelText(NewWizardMessages.VariableCreationDialog_path_dir_button);
	...
	protected void configureShell(Shell newShell) {
		...
		// IJavaHelpContextIds.VARIABLE_CREATION_DIALOG);
	}

	public BPVariableElement getBuildpathElement() {
		return new BPVariableElement(fNameField.getText(), new Path(fPathField
				.getText()));
	}
	...
	private StatusInfo nameUpdated() {
		...
		if (name.length() == 0) {
			status
					.setError(NewWizardMessages.VariableCreationDialog_error_entername);
			return status;
		}
		if (name.trim().length() != name.length()) {
			status
					.setError(NewWizardMessages.VariableCreationDialog_error_whitespace);
		} else if (!Path.ROOT.isValidSegment(name)) {
			status
					.setError(NewWizardMessages.VariableCreationDialog_error_invalidname);
		} else if (nameConflict(name)) {
			status
					.setError(NewWizardMessages.VariableCreationDialog_error_nameexists);
		}
		return status;
	}
	...
	private boolean nameConflict(String name) {
		...
		for (int i = 0; i < fExistingNames.size(); i++) {
			BPVariableElement elem = (BPVariableElement) fExistingNames.get(i);
	...
	private StatusInfo pathUpdated() {
		...
		if (path.length() > 0) { // empty path is ok
			if (!Path.ROOT.isValidPath(path)) {
				status
						.setError(NewWizardMessages.VariableCreationDialog_error_invalidpath);
			} else if (!new File(path).exists()) {
				status
						.setWarning(NewWizardMessages.VariableCreationDialog_warning_pathnotexists);
			}
		}
		return status;
	}
	...
	private String getInitPath() {
		...
		if (initPath.length() == 0) {
			initPath = fDialogSettings.get(IUIConstants.DIALOGSTORE_LASTEXTZIP);
			if (initPath == null) {
				initPath = ""; //$NON-NLS-1$
			}
		} else {
	...
	private IPath chooseExtJarFile() {
		...
		dialog
				.setText(NewWizardMessages.VariableCreationDialog_extzipdialog_text);
	...
	private IPath chooseExtDirectory() {
		...
		dialog
				.setText(NewWizardMessages.VariableCreationDialog_extdirdialog_text);
		dialog
				.setMessage(NewWizardMessages.VariableCreationDialog_extdirdialog_message);
		dialog.setFilterPath(initPath);
		String res = dialog.open();
		if (res != null) {
			fDialogSettings.put(IUIConstants.DIALOGSTORE_LASTEXTZIP, dialog

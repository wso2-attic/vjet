...
import org.eclipse.dltk.mod.ui.viewsupport.ColoredString;
import org.eclipse.dltk.mod.ui.viewsupport.ColoredString.Style;
import org.eclipse.dltk.mod.ui.viewsupport.IRichLabelProvider;
 ...
 * org.eclipse.jdt.internal.ui.viewsupport.OwnerDrawSupport(eclipse 3.3) for
 * supporting eclipse 3.5
 */
public class ColoredViewersManager implements IPropertyChangeListener {
	...
	private static final String PREF_COLORED_LABELS = "colored_labels_in_views"; //$NON-NLS-1$
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (property.equals(QUALIFIER_COLOR_NAME)
				|| property.equals(COUNTER_COLOR_NAME)
	...
	protected final void refreshAllViewers() {
		for (Iterator iterator = fManagedViewers.values().iterator(); iterator
				.hasNext();) {
	...
	private class ManagedViewer implements DisposeListener {
		...
		private final Style DECORATIONS_STYLE = new Style(
				DECORATIONS_COLOR_NAME);
		...
		private ColoredString getColoredLabelForView(Item item) {
			ColoredString oldLabel = (ColoredString) item
			...
			if (newLabel == null) {
				newLabel = new ColoredString(itemText); // fallback. Should
														// never happen.
			} else if (!newLabel.getString().equals(itemText)) {
				// the decorator manager has already queued an new update
				newLabel = decorateColoredString(newLabel, itemText,
						DECORATIONS_STYLE);
			}
		...
		private ColoredString decorateColoredString(ColoredString string,
				String decorated, Style color) {
			String label = string.getString();
			int originalStart = decorated.indexOf(label);
			if (originalStart == -1) {
				return new ColoredString(decorated); // the decorator did
														// something wild
			}
			if (originalStart > 0) {
				ColoredString newString = new ColoredString(
						decorated.substring(0, originalStart), color);
			...
			if (decorated.length() > originalStart + label.length()) { // decorator
				...
				return string.append(
						decorated.substring(originalStart + label.length()),
						color);
			}

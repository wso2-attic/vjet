...
import org.eclipse.dltk.mod.internal.corext.refactoring.RefactoringExecutionStarter;
...
public class RenameModelElementAction extends SelectionDispatchAction {
	...
	public void run(IStructuredSelection selection) {
		...
		try {
			...
			ExceptionHandler.handle(e,
					RefactoringMessages.RenameScriptElementAction_name,
					RefactoringMessages.RenameScriptElementAction_exception);
		}
	}
	...
	public void selectionChanged(ITextSelection selection) {
		if (selection instanceof ModelTextSelection) {
			try {
				IModelElement[] elements = ((ModelTextSelection) selection)
	...
	public void run(ITextSelection selection) {
		...
		MessageDialog.openInformation(getShell(),
				RefactoringMessages.RenameScriptElementAction_name,
				RefactoringMessages.RenameScriptElementAction_not_available);
	}
	...
	private static boolean isRenameAvailable(IModelElement element)
		...
		if (DLTKCore.DEBUG) {
			System.err
					.println("TODO: Add other kind of rename refactoring support..."); //$NON-NLS-1$
		}
		switch (element.getElementType()) {
		case IModelElement.SCRIPT_PROJECT:
			return RefactoringAvailabilityTester
					.isRenameAvailable((IScriptProject) element);
		case IModelElement.PROJECT_FRAGMENT:
			return RefactoringAvailabilityTester
					.isRenameAvailable((IProjectFragment) element);
		case IModelElement.SCRIPT_FOLDER:
			return RefactoringAvailabilityTester
					.isRenameAvailable((IScriptFolder) element);
		case IModelElement.SOURCE_MODULE:
			return RefactoringAvailabilityTester
					.isRenameAvailable((ISourceModule) element);
		case IModelElement.TYPE:
			...
			// return
			// RefactoringAvailabilityTester.isRenameAvailable(method.getDeclaringType());
			// else
			...
			// return
			// RefactoringAvailabilityTester.isRenameEnumConstAvailable(field);
			// else
			// return
			// RefactoringAvailabilityTester.isRenameFieldAvailable(field);
			// case IModelElement.TYPE_PARAMETER:
			// return
			// RefactoringAvailabilityTester.isRenameAvailable((ITypeParameter)
			// element);
			// case IModelElement.LOCAL_VARIABLE:
			// return
			// RefactoringAvailabilityTester.isRenameAvailable((ILocalVariable)
			// element);
		}
		return false;
	}

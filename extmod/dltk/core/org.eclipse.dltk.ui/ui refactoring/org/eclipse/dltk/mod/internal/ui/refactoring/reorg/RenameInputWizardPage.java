...
public abstract class RenameInputWizardPage extends TextInputWizardPage {
	...
	private void addOptionalUpdateReferencesCheckbox(Composite result,
			RowLayouter layouter) {
		final IReferenceUpdating ref = (IReferenceUpdating) getRefactoring()
	...
	private void addOptionalUpdateQualifiedNameComponent(Composite parent,
		...
		fUpdateQualifiedNames
				.setText(RefactoringMessages.RenameInputWizardPage_update_qualified_names);
	...
	private void addOptionalLeaveDelegateCheckbox(Composite result,
			RowLayouter layouter) {
		final IDelegateUpdating refactoring = (IDelegateUpdating) getRefactoring()
				.getAdapter(IDelegateUpdating.class);
		if (refactoring == null || !refactoring.canEnableDelegateUpdating())
			return;
		fLeaveDelegateCheckBox = createCheckbox(result, refactoring
				.getDelegateUpdatingTitle(false), DelegateUIHelper
				.loadLeaveDelegateSetting(refactoring), layouter);
		...
		fLeaveDelegateCheckBox.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				refactoring.setDelegateUpdating(fLeaveDelegateCheckBox
						.getSelection());
			}
		});
	}
	...
	protected void updateLeaveDelegateCheckbox(int delegateCount) {
		...
		fLeaveDelegateCheckBox.setText(refactoring
				.getDelegateUpdatingTitle(delegateCount > 1));
		if (delegateCount == 0) {

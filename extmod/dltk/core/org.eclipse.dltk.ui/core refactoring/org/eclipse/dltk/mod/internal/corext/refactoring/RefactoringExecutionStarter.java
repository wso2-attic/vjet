...
import org.eclipse.dltk.mod.internal.corext.refactoring.reorg.IReorgPolicy.IMovePolicy;
...
public final class RefactoringExecutionStarter {
	public static void startDeleteRefactoring(final Object[] elements,
	...
	public static void startCutRefactoring(final Object[] elements,
			final Shell shell) throws CoreException, InterruptedException,
			InvocationTargetException {
		final ScriptDeleteProcessor processor = new ScriptDeleteProcessor(
				elements);
		processor.setQueries(new ReorgQueries(shell));
		new RefactoringExecutionHelper(new DeleteRefactoring(processor),
				RefactoringCore.getConditionCheckingFailedSeverity(), false,
				shell, new ProgressMonitorDialog(shell)).perform(false);
	}

	public static void startRenameRefactoring(final IModelElement element,
			final Shell shell) throws CoreException {
		// try vjet way

		if (VjetRenameHelper.startRenameModelElement(shell, element, null)) {
	...
	private static RenameSupport createRenameSupport(IModelElement element,
			String newName, int flags) throws CoreException {
		switch (element.getElementType()) {
		case IModelElement.SCRIPT_PROJECT:
			return RenameSupport.create((IScriptProject) element, newName,
					flags);
		case IModelElement.PROJECT_FRAGMENT:
			return RenameSupport.create((IProjectFragment) element, newName);
		case IModelElement.SCRIPT_FOLDER:
			return RenameSupport
					.create((IScriptFolder) element, newName, flags);
		case IModelElement.SOURCE_MODULE:
			return RenameSupport
					.create((ISourceModule) element, newName, flags);
	...
	public static void startRenameResourceRefactoring(final IResource resource,
			final Shell shell) throws CoreException {
		final ScriptRenameRefactoring refactoring = new ScriptRenameRefactoring(
				new RenameResourceProcessor(resource));
		RenameUserInterfaceManager.getDefault().getStarter(refactoring)
				.activate(refactoring, shell, true);
	}

	public static void startRefactoring(final IResource[] resources,
			final IModelElement[] elements, final Shell shell)
			throws ModelException {
		IMovePolicy policy = ReorgPolicyFactory.createMovePolicy(resources,
				elements);
		if (policy.canEnable()) {
			final ScriptMoveProcessor processor = new ScriptMoveProcessor(
					policy);
			final ScriptMoveRefactoring refactoring = new ScriptMoveRefactoring(
					processor);

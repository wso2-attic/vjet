...
public class RenameModelElementDescriptor extends ScriptRefactoringDescriptor {
	...
	public RenameModelElementDescriptor(final String id) {
		...
		// super(fContribution, id, id, id, id, new HashMap(), fMatchStrategy);
		// Assert.isLegal(checkId(id),
		// "Refactoring id is not a rename refactoring id"); //$NON-NLS-1$
	}
	...
	protected void populateArgumentMap() {
		...
		fArguments.put(ScriptRefactoringDescriptor.ATTRIBUTE_NAME, fName);
		...
		// elementToHandle(getProject(), parameter
		// .getDeclaringMember()));
		// fArguments.put(ATTRIBUTE_PARAMETER, parameter.getElementName());
		// } else
		fArguments.put(ScriptRefactoringDescriptor.ATTRIBUTE_INPUT,
				elementToHandle(getProject(), fJavaElement));

		final int type = fJavaElement.getElementType();
		if (type != IModelElement.PROJECT_FRAGMENT)
			fArguments.put(ScriptRefactoringDescriptor.ATTRIBUTE_REFERENCES,
					Boolean.toString(fReferences));
		// if (type == IJavaElement.FIELD) {
		// fArguments.put(ATTRIBUTE_RENAME_GETTER, Boolean
		// .toString(fRenameGetter));
		// fArguments.put(ATTRIBUTE_RENAME_SETTER, Boolean
		// .toString(fRenameSetter));
		// }
		switch (type) {
		case IModelElement.SCRIPT_FOLDER:
		case IModelElement.TYPE:
		case IModelElement.FIELD:
			fArguments.put(ATTRIBUTE_TEXTUAL_MATCHES, Boolean
					.toString(fTextual));
		default:
			break;
		}
		...
		switch (type) {
		case IModelElement.TYPE:
			fArguments.put(ATTRIBUTE_SIMILAR_DECLARATIONS, Boolean
					.toString(fSimilarDeclarations));
			fArguments.put(ATTRIBUTE_MATCH_STRATEGY, Integer
					.toString(fMatchStrategy));
		default:
			break;
		}
		switch (type) {
		case IModelElement.SCRIPT_FOLDER:
			fArguments.put(ATTRIBUTE_HIERARCHICAL, Boolean
					.toString(fHierarchical));
		default:
			break;
		}
	}
	 ...
	 * deprecated.
	 * <p>
	 * Note: Deprecation of the delegate is currently applicable to the Java
	 * elements {IMethod} and {IField}. The default is to not deprecate the
	 * delegate.
	 * </p>
	 * 
	 * @param deprecate
	 ...
	 * </p>
	 * <p>
	 * Note: Qualified name updating is currently applicable to the Java
	 * elements {IPackageFragment} and {IType}. The default is to use no file
	 ...
	 * {IJavaElement#JAVA_PROJECT}, clients are required to to set the project
	 * name to <code>null</code>.
	 * </p>
	 * 
	 * @param element
	 ...
	 * <p>
	 * Note: Keeping of original elements as delegates is currently applicable
	 * to the Java elements {IMethod} and {IField}. The default is to not keep
	 ...
	 * </p>
	 * <p>
	 * Note: Similar declaration updating is currently applicable to Java
	 * elements of type {IType}. The default is to use the
	 * {@link #STRATEGY_EXACT} match strategy.
	 * </p>
	 * 
	 * @param strategy
	...
	public void setMatchStrategy(final int strategy) {
		Assert.isLegal(
				strategy == STRATEGY_EXACT || strategy == STRATEGY_EMBEDDED
	 ...
	 * <p>
	 * Note: If the Java element to be renamed is of type clients are required
	 * to to set the project name to <code>null</code>.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Note: Renaming of getter methods is applicable for {IField} elements
	 * which do not represent enum constants only. The default is to not rename
	 * any getter methods.
	 * </p>
	 * 
	 * @param rename
	 ...
	 * <p>
	 * Note: Renaming of setter methods is applicable for IField elements which
	 * do not represent enum constants only. The default is to not rename any
	 * setter methods.
	 * </p>
	 * 
	 * @param rename
	 ...
	 * type. The default is to not update Java elements hierarchically.
	 * </p>
	 * 
	 * @param update
	 ...
	 * <p>
	 * Note: Qualified name updating is currently applicable to the Java
	 * elements {IPackageFragment} and {IType}. The default is to not rename
	 * qualified names.
	 * </p>
	 * 
	 * @param update
	 ...
	 * <p>
	 * Note: Reference updating is currently applicable to all Java element
	 * types except {IPackageFragmentRoot}. The default is to not update
	 * references.
	 * </p>
	 * 
	 * @param update
	 ...
	 * <p>
	 * Note: Similar declaration updating is currently applicable to Java
	 * elements of type {IType}. The default is to not update similar
	 * declarations.
	 * </p>
	 * 
	 * @param update
	 ...
	 * </p>
	 * <p>
	 * Note: Textual occurrence updating is currently applicable to the Java
	 * elements {IPackageFragment}, {IType} and {IField}. The default is to not
	 * rename textual occurrences.
	 * </p>
	 * 
	 * @param update
	...
	// .createFatalErrorStatus(DescriptorMessages.RenameResourceDescriptor_no_new_name));
	// if (fJavaElement == null)
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_no_java_element));
	// else {
	// final int type = fJavaElement.getElementType();
	// if (type == IJavaElement.JAVA_PROJECT && getProject() != null)
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_project_constraint));
	// if (type == IJavaElement.PACKAGE_FRAGMENT_ROOT && fReferences)
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_reference_constraint));
	// if (fTextual) {
	// switch (type) {
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_textual_constraint));
	// }
	// }
	// if (fDeprecate) {
	// switch (type) {
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_deprecation_constraint));
	// }
	// }
	// if (fDelegate) {
	// switch (type) {
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_delegate_constraint));
	// }
	// }
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_accessor_constraint));
	// }
	// if (fQualified) {
	// switch (type) {
	// case IJavaElement.PACKAGE_FRAGMENT:
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_qualified_constraint));
	// }
	// }
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_similar_constraint));
	// }
	// }
	// if (fHierarchical) {
	// switch (type) {
	...
	// status
	// .merge(RefactoringStatus
	// .createFatalErrorStatus(DescriptorMessages.RenameJavaElementDescriptor_hierarchical_constraint));
	// }
	// }
	// }
	// return status;

...
public interface IType extends IMember {
	 ...
	 * <code>"bar"</code>). This is a handle-only method. The field may or may
	 * not exist.
	 * 
	 * @param name
	 *            the given name
	 ...
	 * types, this may include the special <code>&lt;clinit&gt;</code>; method
	 ...
	 * followed by <code>'.'</code>, followed by the type-qualified name using
	 * the <code>enclosingTypeSeparator</code>. This is a handle-only method.
	 * 
	 ...
	 * @deprecated this methods prepends type name with package (folder) but
	 *             this is needed only in java
	 */
	String getFullyQualifiedName(String enclosingTypeSeparator);

	/**
	 * @return this methods prepends type name with package (folder) but this is
	 *         needed only in java
	 */
	 ...
	 * @param position
	 *            the position within snippet where the user is performing code
	 *            assist.
	 ...
	 * 
	 */
	void codeComplete(char[] snippet, int insertion, int position,
	...
	/**
	 * Do code completion inside a code snippet in the context of the current
	 * type. It considers types in the working copies with the given owner
	 * first. In other words, the owner's working copies will take precedence
	 * over their original compilation units in the workspace.
	 * <p>
	 ...
	 * or the occurence count of this type if it is anonymous. For binary types,
	 * this is the name of the class file without the ".class" suffix. This is a
	 * handle-only method.
	 * 
	 ...
	 * any enclosing types, but not including package qualification. For source
	 * types, this consists of the simple names of any enclosing types,
	 * separated by <code>enclosingTypeSeparator</code>, followed by the simple
	 * name of this type or the occurence count of this type if it is anonymous.
	 ...
	 * <li>m1 has the same number of arguments as m2 and the simple names of the
	 * argument types must be equals.
	 * <li>m1 exists.
	 * </ul>
	 * 
	 * @param method
	 ...
	 *                while accessing its corresponding resource.
	 */
	ITypeHierarchy newSupertypeHierarchy(ISourceModule[] workingCopies,
			IProgressMonitor monitor) throws ModelException;

	/**
	 * Creates and returns a type hierarchy for this type containing this type
	 * and all of its supertypes, considering types in the working copies with
	 * the given owner. In other words, the owner's working copies will take
	 ...
	 *         project
	 */
	ITypeHierarchy newTypeHierarchy(IScriptProject project,
			IProgressMonitor monitor) throws ModelException;

	/**
	 * Creates and returns a type hierarchy for this type containing this type,
	 * all of its supertypes, and all its subtypes in the context of the given
	 * project, considering types in the working copies with the given owner. In
	 * other words, the owner's working copies will take precedence over their
	 ...
	 *                while accessing its corresponding resource.
	 */
	ITypeHierarchy newTypeHierarchy(ISourceModule[] workingCopies,
			IProgressMonitor monitor) throws ModelException;

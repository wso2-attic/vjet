...
public class ExternalScriptFolder extends ScriptFolder {
	...
	public ISourceModule getSourceModule(String name) {
		...
		// ExternalEntryFile storage = new
		// ExternalEntryFile(EnvironmentPathUtils.getFile(path));
		// return new ExternalSourceModule(this, name,
		// DefaultWorkingCopyOwner.PRIMARY, storage);
		return createExternalSourceModule(name);
		// EBAY - END MOD
	}
	...
	private ISourceModule createExternalSourceModule(String name) {
		...
		if (factory != null) {
			sourceModule = factory.createExternalSourceModule(this, name,
					DefaultWorkingCopyOwner.PRIMARY, true, storage);
		} else {
			sourceModule = new ExternalSourceModule(this, name,
					DefaultWorkingCopyOwner.PRIMARY, storage);
		}
		return sourceModule;
	}

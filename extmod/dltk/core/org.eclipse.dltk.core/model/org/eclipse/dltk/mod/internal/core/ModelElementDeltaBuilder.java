	...
	private void findContentChange(ModelElementInfo oldInfo,
		...
		if (oldInfo instanceof MemberElementInfo
			...
			if (((MemberElementInfo) oldInfo).getModifiers() != ((MemberElementInfo) newInfo)
				...
				if (oldInfo instanceof JSSourceMethodElementInfo
						&& newInfo instanceof JSSourceMethodElementInfo) {
					// add support for vjet source method
					JSSourceMethodElementInfo oldSourceMethodInfo = (JSSourceMethodElementInfo) oldInfo;
					JSSourceMethodElementInfo newSourceMethodInfo = (JSSourceMethodElementInfo) newInfo;
			...
			} else if (oldInfo instanceof JSSourceFieldElementInfo
					&& newInfo instanceof JSSourceFieldElementInfo) {
				// add support for vjet source field
				if (!CharOperation.equals(((JSSourceFieldElementInfo) oldInfo)
						.getType().toCharArray(),
						((JSSourceFieldElementInfo) newInfo).getType()
								.toCharArray())) {
					this.delta

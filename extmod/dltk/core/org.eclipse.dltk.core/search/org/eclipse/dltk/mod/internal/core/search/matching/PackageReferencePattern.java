...
public class PackageReferencePattern extends AndPattern implements
		IIndexConstants {
	...
	public PackageReferencePattern(char[] pkgName, int matchRule,
		...
		if (pkgName == null || pkgName.length == 0) {
			...
			this.pkgName = (isCaseSensitive() || isCamelCase()) ? pkgName
	...
	PackageReferencePattern(int matchRule, IDLTKLanguageToolkit toolkit) {
		super(PKG_REF_PATTERN, matchRule, toolkit);
	}

	public void decodeIndexKey(char[] key) {
		// Package reference keys are encoded as 'name' (where 'name' is the
	...
	protected boolean hasNextQuery() {
		...
		// they are mostly
		// redundant (eg. in 'org.eclipse.jdt.core.*' 'org.eclipse' is used all
		// the time)

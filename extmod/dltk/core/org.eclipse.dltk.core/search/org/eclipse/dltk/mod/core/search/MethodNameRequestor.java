...
/**
 * A <code>MethodNameRequestor</code> collects search results from a
 ...
 * <code>SearchEngine.searchAllTypeNames(...)</code> method. Only top-level and
...
public abstract class MethodNameRequestor {
	 ...
	 *            these flags may slightly differ from thoses get after
	 *            resolution. For example an interface defined by
	 *            <code>interface A {}</code>, although obviously public, will
	 *            be returned false by <code>Flags.isPublic(modifiers)</code>
	 *            due to the fact that its declaration does not explicitely
	 *            define public flag.
	 * @param packageName
	 *            the dot-separated name of the package of the type
	 ...
	 *            archive (that is, a .zip file), the path is composed of 2
	 *            paths separated by
	 *            <code>IDLTKSearchScope.ARCHIVE_FILE_ENTRY_SEPARATOR</code>:
	 *            the first path is the full OS path to the archive (if it is an

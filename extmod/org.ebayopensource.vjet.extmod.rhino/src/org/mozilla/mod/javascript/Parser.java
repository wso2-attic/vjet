...
// end of per function variables
        ...
        throws IOException, ParserException
    {
        ...
        if (matchToken(Token.NAME)) {
            ...
            // Anonymous function
            name = "";
			// EBAY MOD -- want to get name of function

			/*
			 * if assigned to prototype use Object.prototype.functionName =
			 * function(){} set name to String_prototype_functionName
			 */
			if(Context.getContext().isCreateMetaData()){
				name = determineFunctionNameFromStatement(ts.getLine(), ts.getString());
			}
			// END EBAY MOD
        } else {
            name = "";
		...
		// EBAY MOD
		boolean createMetaData = Context.getContext().isCreateMetaData();
		if (createMetaData) {
			String sourceString = ts.getSourceString();
			String source = "";
			String jsdocs = "";
			if (sourceString != null) {
				source = subStringFromLineNums(sourceString, baseLineno,
						ts.getLineno());
				jsdocs = findJsDocFromLineNum(sourceString, baseLineno);
			}

			// jsdocs = ts.lastJsDoc.toString();
			fnNode.setSourceCode(source);
			fnNode.setJsDocs(jsdocs);

			//System.out.println("jsdocs = \n" + jsdocs + "\nEND JS DOCS\n");
    ...
    //EBAY MOD
	private String determineFunctionNameFromStatement(String line, String funcShortName) {		
		if(line.indexOf("function")==-1){			
			return "";			
		}
		String sanitizedLine = getSanitizedLineWithFunction(funcShortName, line);
		
		//int endIndex = line.indexOf(funcShortName)+funcShortName.length();
		StringBuilder temp = new StringBuilder();
		
		String[] strAry = sanitizedLine.split("=");
		String name = "";
		name = strAry[0];
		if (name != null) {
			name = name.trim();
			String[] tokenAry = name.split("\\.");
			for (int i = 0; i < tokenAry.length; i++) {
				String obj = tokenAry[i];
				if(obj.indexOf("function")==-1){
					temp.append(obj);
					if (i < tokenAry.length-1) {
						temp.append(".");
					}
				}
			}
	...
	private String getSanitizedLineWithFunction(String funcShortName, String line) {
		if(line.indexOf(";")==-1){
			return line;
		}
		String[] lines = line.split(";");
		for(String aline: lines){
			if(aline.indexOf("." + funcShortName)!=-1){
				return aline;
			}
		}
		return "";
	...
	private String findJsDocFromLineNum(String sourceString, int baseLineno) {
		assert sourceString != null;
		assert sourceString.indexOf("/**")!=-1;
		StringBuilder jsDoc = new StringBuilder();
		String[] lines = sourceString.split("\n"); // TODO Auto-generated
													// method stub
		boolean foundStartJsDocComment = false;
		int jsDocStartLine = baseLineno;
		int jsDocEndLine = baseLineno;
		int linePos = baseLineno;
		do {
			linePos--;
			if(linePos<0){
				// found nothing
			...
			if (linePos <= 0) {
				...
				adjustedIndx = linePos;
			}

			String curLine = lines[adjustedIndx];
			if(curLine.contains("}")){ // end of function
				// short circuit function
				foundStartJsDocComment = true;
			}
			if (curLine.contains("/**")) {
				foundStartJsDocComment = true;
				jsDocStartLine = linePos;
			} else if (curLine.contains("*/")) {
				jsDocEndLine = linePos + 1;
			}
		} while (foundStartJsDocComment == false);

		for (int itr = jsDocStartLine; itr < jsDocEndLine; itr++) {
			jsDoc.append(lines[itr]);
			jsDoc.append("\n");
		}
	...
	private String subStringFromLineNums(String sourceContent, int startingLine, int endingLine) {
		assert sourceContent != null;
		StringBuilder temp = new StringBuilder();
		String[] lines = sourceContent.split("\n");

		int xEndingLine = endingLine;
		for (int i = startingLine; i <= xEndingLine; i++) {
			if(i<lines.length){
				temp.append(lines[i]);
				temp.append("\n");
			}

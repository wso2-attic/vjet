                                                  ...
                                                  ConstProperties
{
    ...
    //EBAY MOD : private static class Slot implements Serializable
	...
	public Slot[] getSlots() {
		int size = (count >= 0) ? count : ~count;
		if (size == 0) {
			return EMPTY_SLOTS;
		}
		List<Slot> slotList = new ArrayList<Slot>(size);
		for (Slot slot : slots) {
			while (slot != null) {
				slotList.add(slot);
				slot = slot.next;
		...
		return slotList.toArray(EMPTY_SLOTS);
	}

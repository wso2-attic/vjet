...
public class Parser implements  ParserBasicInformation, TerminalTokens, OperatorIds, TypeIds {
	...
	private static boolean DEBUG_AUTOMATON = false;
	...
	private final static String READABLE_NAMES_FILE_NAME =
		"org.eclipse.mod.wst.jsdt.internal.compiler.parser." + READABLE_NAMES_FILE; //$NON-NLS-1$
...
//			if(hasError){
//			System.out.println("RECOVERYING");
//			}
		...
		if (act <= NUM_RULES) {
		    ...
		    if (act < ACCEPT_ACTION) { /* shift */
				...
				try{
					...
					insertedSemicolon = false;
					this.currentToken = this.scanner.getNextToken();
					if (DEBUG){ if(scanner.dumpCurrent()!=null){ System.out.println(">>shift next Token: "+scanner.dumpCurrent());}} //$NON-NLS-1$
				} catch(InvalidInputException e){
					if (!this.hasReportedError){
						this.problemReporter().scannerError(this, e.getMessage());
						this.hasReportedError = true;
...
private BalanceUtil preParse() {	
	...
	do {
		try {
			currTok = this.scanner.getNextToken();
			balance.addToken(currTok, scanner.startPosition);
		} catch (InvalidInputException e) {
			return null;
		}		
	} while (currTok!=TerminalTokens.TokenNameEOF);
		
	this.scanner.m_fakeIdentifierProvider = new FakeIdentifierProvider(
		balance.getTuplesToBeInserted(), balance.getTuplesToBeIgnored());
	
	// reset scanner
	this.scanner.resetTo(0, scanner.source.length-1);	
	return balance;
}
/** END EBAY MOD **/
...
//		initializeInferenceEngine(this.compilationUnit);
		...
		/* run automaton */
		BalanceUtil balance = preParse(); // EBAY MOD
		
		parse();
		
		// START EBAY MOD
		m_hasNonFakeTokenInsertionError = compilationResult.hasErrors();
		if (balance != null) {
			Map<Integer,List<Tuple>> insertedTuples = balance.getTuplesToBeInserted();
			if (!insertedTuples.isEmpty()){
				for (List<Tuple> tuples : insertedTuples.values()) {
					if (!tuples.isEmpty()) {
						Tuple t = tuples.get(0); //only report first error for incomplete "." or ":"
						char[] tokenSrc = BalanceUtil.getTokenSource(t.m_previousToken);
						problemReporter().parseErrorInsertAfterToken(
							t.m_location, t.m_location, t.m_previousToken,
							tokenSrc, new String(tokenSrc), IDENTIFIER);
					}
				}
			}
			Map<Integer,Tuple> ignoredTuples = balance.getTuplesToBeIgnored();
			if (!ignoredTuples.isEmpty()){
				for (Tuple t : ignoredTuples.values()) {
					char[] tokenSrc = BalanceUtil.getTokenSource(t.m_token);
					problemReporter().parseErrorInsertAfterToken(
						t.m_location, t.m_location, t.m_token,
						tokenSrc, new String(tokenSrc), IDENTIFIER);
				}
			}
		}
...
//		TypeDeclaration[] types = this.compilationUnit.types;
		int[][] intervalToSkip = org.eclipse.mod.wst.jsdt.internal.compiler.parser.diagnose.RangeUtil.computeDietRange(statements);
